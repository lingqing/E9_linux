...
#define SABRESD_GPIO_VOL_UP     IMX_GPIO_NR(4, 8)   //left
#define SABRESD_GPIO_VOL_DOWN   IMX_GPIO_NR(4, 9)   //right
#define SABRESD_GPIO_POWER      IMX_GPIO_NR(4, 5)   //power
#define BOARD_POWER_CTRL        IMX_GPIO_NR(6, 31)  //power ctrl

//#define SABERSD_GPIO_DS18B20    IMX_GPIO_NR(1, 29)
...
#define SABRESD_ECSPI1_CS0      IMX_GPIO_NR(4, 13)
...
static void mx6q_csi0_cam_powerdown(int powerdown)
{
	...
	for (;;)
	{
		...
		msleep(1000);
		gpio_set_value(SABRESD_CSI0_PWN,0);					//cam0 off
 		msleep(1000);
		gpio_set_value(SABRESD_CSI0_RST,1);					//cam0 off
		msleep(1000);
		gpio_set_value(SABRESD_CSI0_RST,0);					//cam0 off
		msleep(1000);
		printk(".");
	}
#endif
	if(powerdown != 0){		//off
		//log_msg(LOG_DEBUG,"camera power off\n");
		gpio_set_value(SABRESD_CSI0_PWN,1);					//cam0 off
	}
	else{											//cam1 not refrect 	
		//log_msg(LOG_DEBUG,"camera power on\n");
...
static void tq_capts_poweron(int onoff)
{
	...
	//log_msg(LOG_DEBUG,"cap set %s",onoff!=0?"on":"off");
	err = gpio_request(SABRESD_GPIO_TPPWN,"cap_ts_power\n");
	if(err<0)
	{
		//log_msg(LOG_DEBUG,"can't request power gpio %d\n",SABRESD_GPIO_TPPWN);
		return ;
	}
	if(onoff != 0)
...
static struct i2c_board_info mxc_i2c1_board_info[] __initdata = {
	...
	{
		I2C_BOARD_INFO("ov5640", 0x3c),//0x78 0x3c
...
//	soc_reg_id = sabresd_dvfscore_data.soc_id;
	...
	strcpy(mxc_i2c1_board_info[1].type, "wm8960");
	mxc_i2c0_board_info[1].platform_data = &wm8960_config_data;
